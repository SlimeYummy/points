...
impl ScriptExecutor {
    pub fn new() -> Box<ScriptExecutor> {
        let mut executor = Box::new(ScriptExecutor {
            pc: 0,
        ...
        executor.num_segs.set_segment(SEGMENT_REGISTER, unsafe {
            slice::from_raw_parts_mut(executor.registers.as_mut_ptr() as *mut u64, MAX_REGISTER)
        });
        executor
    }
    ...
    pub fn run_hook<const I: usize, const O: usize, C: ScriptEnv<I, O>>(
        ...
        self.execute_loop(&hook_block.code, context)
    }
    ...
    fn read_str(&self, addr: &CmdAddr) -> Symbol {
        self.str_seg.get(addr.segment(), addr.offset())
    }
    ...
    fn x_set(global: &mut SymbolMap<Num>, k: Symbol, v: Num) -> Num {
        global.insert(k, v).unwrap_or(0.0)
    }
...
impl NumberSegments {
    ...
    fn get(&self, seg: u8, off: u16) -> u64 {
        unsafe { self.num_segs[seg as usize][off as usize] }
    }
...
impl StringSegment {
    ...
    fn get(&self, seg: u8, off: u16) -> Symbol {
        ...
        self.str_seg[off as usize].clone()
    }
}
...
mod tests {
    ...
    pub fn new_blocks(typ: ScriptBlockType, code: ScriptByteCode) -> ScriptBlocks {
        ScriptBlocks::new(vec![ScriptBlock::new_hook(typ, code).unwrap()]).unwrap()
    }
    ...
    fn test_executor_call() {
        ...
        executor.run_hook(&blocks, BeforeHit, &mut ctx).unwrap();
        ...
        executor.registers[2] = 32.0;
        ctx.in2.gg = 55.5;
        executor.run_hook(&blocks, AfterAssemble, &mut ctx).unwrap();
        ...
        blocks.constant_segment.push(unsafe { mem::transmute(111.0) });
        executor.registers[12] = 1.0;
        executor.registers[13] = 21.0;
        executor.registers[14] = 31.0;
        ctx.in2.gg = 55.5;
        executor.run_hook(&blocks, AfterAssemble, &mut ctx).unwrap();
    ...
    fn test_executor_jmp() {
        ...
        ctx.in1.bb = 3.0;
        ctx.closure_segment[0] = unsafe { mem::transmute(-5.0) };
        executor.run_hook(&blocks, OnAssemble, &mut ctx).unwrap();
    ...
    fn test_executor_jmp_cmp() {
        ...
        ctx.out.xx = 0.0;
        executor.run_hook(&blocks, OnTreat, &mut ctx).unwrap();
        assert_eq!(ctx.out.xx, 2.0);
