...
#[derive(Debug, Default)]
pub struct TestEnv {
    pub closure_segment: [Num; MAX_CLOSURE],
    pub in1: TestIn1,
    pub in2: TestIn2,
    pub out: TestOut,
        ...
        return [
            unsafe {
                slice::from_raw_parts(
                    &self.in1 as *const _ as *const u64,
                    mem::size_of::<TestIn1>() / mem::size_of::<u64>(),
                )
            },
            unsafe {
                slice::from_raw_parts(
                    &self.in2 as *const _ as *const u64,
                    mem::size_of::<TestIn2>() / mem::size_of::<u64>(),
                )
            },
        ];
    }
    ...
    fn out_segments(&mut self) -> [&mut [u64]; 1] {
        return [unsafe {
            slice::from_raw_parts_mut(
                &mut self.out as *mut _ as *mut u64,
                mem::size_of::<TestOut>() / mem::size_of::<u64>(),
            )
        }];
    }
...
pub fn new_parser() -> ScriptParser {
    ...
    for block_typ in enum_iterator::all::<ScriptBlockType>() {
        ...
        if block_typ.is_timer() {
            inputs.insert(
                (block_typ, SEGMENT_IN_MIN + 1),
                HashMap::from([("in.hh".into(), 0), ("in.ii".into(), 1)]),
            );
        }
    }
    ...
    for block_typ in enum_iterator::all::<ScriptBlockType>() {
        outputs.insert(
            (block_typ, SEGMENT_OUT_MIN),
            HashMap::from([
                ("out.xx".into(), (0, ScriptOutType::Mov)),
                ("out.yy".into(), (1, ScriptOutType::Add)),
                ("out.zz".into(), (2, ScriptOutType::Mul)),
            ]),
        );
    }

    let mut expr_funcs = HashMap::new();
    for block_typ in enum_iterator::all::<ScriptBlockType>() {
        use CmdType::*;
        expr_funcs.insert(
            block_typ,
            HashMap::from([
                ("ext.f1".into(), (1, vec![Str])),
                ("ext.f2".into(), (2, vec![Str, Num])),
                ("ext.f3".into(), (3, vec![Str, Num, Num])),
                ("ext.f4".into(), (4, vec![Str, Num, Num, Str])),
            ]),
        );
    }
...
// static CONSTS_MAP: SyncLazy<HashMap<&'static str, Num>> = SyncLazy::new(|| {
//     let mut map = HashMap::new();
//     map.insert("PI", Num::pi());
//     map.insert("E", Num::e());
//     map.insert("TAU", Num::tau());
//     map.insert("MAX", Num::max_value());
//     map.insert("MIN", Num::min_value());
//     return map;
// });
