...
unsafe impl LogicAiTaskAny for LogicAiTaskPatrol {
    ...
    fn update(&mut self, ctx: &mut ContextUpdate, ctxt: &mut ContextAiTask) -> XResult<AiTaskReturn> {
        ...
        let ret = match self.mode {
            ...
            AiTaskPatrolMode::MoveStop => Some(self.update_move_stop(ctx, ctxt)?),
        };
        match ret {
            Some(ret) => Ok(ret),
...
impl LogicAiTaskPatrol {
    #[inline]
    fn init_mode(&mut self, mode: AiTaskPatrolMode) {
        self.mode = mode;
        self.idle_timer = 0.0;
        self.path_idx = 0;
        if mode != AiTaskPatrolMode::Move {
            self.move_path = None;
        }
    }
    ...
    fn update_move_stop(&mut self, ctx: &mut ContextUpdate, ctxt: &mut ContextAiTask) -> XResult<AiTaskReturn> {
        ...
        if is_inactive {
            self.init_mode(AiTaskPatrolMode::Idle);
            if self.current_action != self.inst_idle.tmpl_id {
            ...
            match self.inst.route[self.step_idx as usize] {
                InstAiTaskPatrolStep::Move(point) => {
                    if Self::is_reached(ctxt.chara_phy.position(), point, self.inst_move.min_distance) {
                        ...
                        self.init_mode(AiTaskPatrolMode::Move);
                        if self.current_action != self.inst_move.tmpl_id {
                ...
                InstAiTaskPatrolStep::Idle(duration) => {
                    ...
                    if self.mode == AiTaskPatrolMode::Move {
                        self.init_mode(AiTaskPatrolMode::MoveStop);
                        if self.current_action != self.inst_move.tmpl_id {
                            self.current_action = self.inst_move.tmpl_id;
                            ret.next_action = Some(self.inst_move.clone());
                        }
    ...
    fn guess_threshold_xz(&self) -> f32 {
        ...
        match self.inst.route[next_idx as usize] {
            InstAiTaskPatrolStep::Move(_) => self.inst_move.step_length * THRESHOLD_XZ_RATIO_MOVE,
            InstAiTaskPatrolStep::Idle(_) => self.inst_move.step_length * THRESHOLD_XZ_RATIO_IDLE,
        }
    }
...
mod tests {
    ...
    use crate::template::TmplAiTaskPatrol;
    use crate::utils::id;
    ...
    //         .systems
    //         .tmpl_db
    //         .find_as::<TmplAiTaskPatrol>(id!("AiTask.InstanceNpc.Idle^1"))
    //         .unwrap();
    //     let inst_task = Rc::new(InstAiTaskPatrol::new(tmpl_task));
    ...
    //     let mut logic = LogicAiTaskPatrol::new(&mut ctx, inst_task.clone(), inst_chara.clone()).unwrap();
    ...
    //     logic.path_idx = 1;
    //     logic.mode = AiTaskPatrolMode::Move;

    //     let state = logic.save();
    //     let mut logic2 = LogicAiTaskPatrol::new(&mut ctx, inst_task, inst_chara).unwrap();

    //     logic2.restore(state.as_ref()).unwrap();

    //     assert_eq!(logic2.move_path, path);
    //     assert_eq!(logic2.path_idx, 1);
    //     assert_eq!(logic2.mode, AiTaskPatrolMode::Move);
    // }
}
