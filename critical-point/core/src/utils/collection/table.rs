...
impl<K, V> Table<K, V> {
    ...
    pub fn len(&self) -> usize {
        return self.header_len as usize;
    }
...
const _: () = {
    ...
    impl<'de, 't, K: DeserializeOwned, V: DeserializeOwned> Visitor<'de> for SeqItemVisitor<'t, K, V> {
        ...
        fn visit_map<A: MapAccess<'de>>(self, mut seq: A) -> Result<K, A::Error> {
            ...
            return key.ok_or_else(|| Error::custom("missing key"));
        }
    }
};
...
mod tests {
    ...
    fn test_table_serde() {
        ...
        assert_eq!(tb2.len(), 3);
        assert_eq!(
            tb2.key_iter().map(|x| x.clone()).collect::<Vec<Symbol>>(),
            vec![s!("k1"), s!("k2"), s!("k3")]
        );
        assert_eq!(
