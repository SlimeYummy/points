...
use crate::utils::{rkyv_self, xerr, xfromf, xres, xresf, DeterministicState, IdentityState, XError, XResult};
...
fn test_init_id_static() {
    unsafe { init_id_static("../../test-tmp/resource").unwrap() };
}
...
impl TmplSymbolCache {
    pub(crate) fn from_file<P: AsRef<Path>>(path: P) -> XResult<TmplSymbolCache> {
        ...
        if fs::exists(&rkyv_path).unwrap_or(false) {
            ...
            let strings = unsafe { rkyv::archived_root::<Vec<String>>(&buf) };
...
impl TmplID {
    ...
    fn new_with(s: &str, cache: &TmplSymbolCache) -> XResult<TmplID> {
        let segs = Self::split_str(s).ok_or_else(|| xerr!(BadArgument))?;
        ...
        let suffix = Self::parse_suffix(segs[4]).ok_or_else(|| xerr!(BadArgument))?;
...
macro_rules! id {
    ($string:expr) => {
        $crate::template2::id::TmplID::new($string).unwrap()
    };
    ($($arg:tt)*) => {{
        let res = std::fmt::format(format_args!($($arg)*));
        $crate::template2::TmplID::new(&res).unwrap()
    }}
}
pub use id;
use regex::Regex;
...
mod tests {
    ...
    fn test_tmpl_id_json_rkyv() {
        ...
        let buf = rkyv::to_bytes::<_, 128>(&id3).unwrap();
        let id4 = unsafe { rkyv::archived_root::<TmplID>(&buf) };
        let id5 = id4.deserialize(&mut rkyv::Infallible).unwrap();
